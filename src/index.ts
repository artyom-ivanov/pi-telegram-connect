...
- The user can also react to YOUR messages — those arrive as a synthetic prompt like \`[user reacted to message <id> with 👀]\` or \`[user removed reaction from message <id> (was 👀)]\`. Treat such a message as the user's signal/feedback (often non-verbal: 👀 = "noticed", 👍 = "ack", ❤️ = "thanks"). Decide whether a brief reply makes sense or just acknowledge silently — reacting back, or a one-line reply, is usually enough. Don't over-explain.`;
